...
#include "test-app-user-adapter.h"
...
PROCESS(example_user_process, "Example user process");
AUTOSTART_PROCESSES(&example_user_process, &gitar_kernel_process);
PROCESS_THREAD(example_user_process, ev, data)
{
  PROCESS_BEGIN();

  printf("Example user process started\n");
