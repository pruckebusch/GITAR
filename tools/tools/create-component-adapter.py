...
PROVIDED_FUNCTIONS = subprocess.check_output("msp430-nm "+ options.objfile+" | grep T | cut -f3 -d "+'" "',shell=True).split("\n")
list.pop(PROVIDED_FUNCTIONS)
print "PROVIDED_FUNCTIONS: ["+ ",".join(PROVIDED_FUNCTIONS) + "]"

REQUIRED_FUNCTIONS = subprocess.check_output("msp430-nm "+ options.objfile+" | grep U |  awk '{print $2}' ",shell=True).split("\n")
list.pop(REQUIRED_FUNCTIONS)
print "REQUIRED_FUNCTIONS: ["+ ",".join(REQUIRED_FUNCTIONS) + "]"

PROVIDED_PROCESSES = subprocess.check_output("msp430-nm "+ options.objfile+" | grep D | cut -f3 -d "+'" "',shell=True).split("\n")
list.pop(PROVIDED_PROCESSES)
print "PROVIDED_PROCESSES: ["+ ",".join(PROVIDED_PROCESSES) + "]"

PROVIDED_DATA = subprocess.check_output("msp430-nm "+ options.objfile+" | grep C | cut -f3 -d "+'" "',shell=True).split("\n")
list.pop(PROVIDED_DATA)
print "PROVIDED_DATA: ["+ ",".join(PROVIDED_DATA) + "]"
